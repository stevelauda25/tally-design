# AGENTS.md — tally-ui Design System
...
→ Trigger full autonomous sync per `.Codex/commands/sync-figma.md`. **No
...
These commands are pre-allowed in `.Codex/settings.json` and should be
...
- **`/new-item <figma-url>`** — BOOTSTRAP a new Figma node into tracking.
  ...
  for ongoing work. See `.Codex/commands/new-item.md`.
- **`/sync-figma <slug>`** — pull from Figma, apply edits, bless, verify. See
  `.Codex/commands/sync-figma.md`. Sat-set; never modifies sacred tokens.
...
- **`/publish [patch|minor|major|x.y.z]`** — verify in-sync + clean tree → build →
  ...
  `.Codex/commands/publish.md`. Pauses ONCE before actual publish.
